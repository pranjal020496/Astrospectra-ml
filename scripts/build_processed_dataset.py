...
MANIFEST_FILE = Path("data/raw/dataset/manifest.csv")
OUTPUT_FILE = Path("data/processed/small_dataset.npz")
...
def read_manifest() -> list[dict[str, str]]:
    """Read spectrum information from the dataset manifest."""

    if not MANIFEST_FILE.exists():
        raise FileNotFoundError(
            f"Manifest not found: {MANIFEST_FILE}. "
            "Run download_small_dataset.py first."
        )
    ...
    if not rows:
        raise ValueError("The manifest contains no spectra.")

    return rows


def main() -> None:
    """Build a fixed-length machine-learning dataset."""
    ...
    for number, row in enumerate(
        ...
        object_class = row["object_class"].strip().upper()
        ...
        try:
            spectrum = load_sdss_spectrum(file_path)
        ...
        feature_rows.append(
            processed.flux.astype(np.float32)
        )

        mask_rows.append(
            processed.valid_mask.astype(bool)
        )
